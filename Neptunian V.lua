...
-- Credits to NoobyGames12 ---------

-- Soundtrack
if not isfile("nep.mp3") then 
	writefile("nep.mp3",game:HttpGet("https://github.com/gObl00x/Soundtracks/raw/refs/heads/main/nep.mp3"))
 end
...
local muter = false
local ORGID = getcustomasset("nep.mp3")
local ORVOL = 1.15
local ORPIT = 1.01
...
--
FELOADLIBRARY = {}
loadstring(game:HttpGet("https://raw.githubusercontent.com/Nitro-GT/loadlibrary/refs/heads/main/main", true))()
...
do
	...
	swait = task.wait -- temp definition

	task.spawn(function()

		local coroutine, os, math = coroutine, os, math

		local fire, yield, resume

		function fire()
			for thread, value in Threads do
				resume(thread)
			end
		end

		function yield(thread: thread)
			Threads[thread] = true
		...
		function swait(delay: number?)
			if not delay then delay = 0 end
			for index = 0, delay do
		...
		while true do
			...
			if Frames >= Frame then
				if AllowFrameLoss then
					...
					for i = 1, math.floor(Frames / Frame) do
						fire()
					end
					LastFrame = os.clock()
				end
				if TossRemainder then
					...
					Frames -= Frame * math.floor(Frames / Frame)
				end
			end
		end
	end)
...
while true do
	...
	local Loudness = kan.PlaybackLoudness
	...
	imgl2.Rotation = imgl2.Rotation - Loudness/50
	imgl2.ImageColor3 = Color3.new(0.15 + Loudness/2500,0,0.6 + Loudness/1000)
	imgl2b.Rotation = imgl2b.Rotation + Loudness/25
	imgl2b.ImageColor3 = Color3.new(0,0.3 + Loudness/1500,0.6 + Loudness/1000)
